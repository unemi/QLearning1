...
@implementation Display {
	...
	id<MTLBuffer> vxBuf[2], colBuf[2];
	NSInteger cBufIdxD, cBufIdxR, vBufIdxD, vBufIdxR;
	BOOL cBufModified, vBufModified, renderCompleted;
	NSLock *bufLockD, *bufLockR;
...
- (void)adjustColBufferSize:(id<MTLBuffer> *)newColBuf {
	newColBuf[0] = colBuf[0]; newColBuf[1] = colBuf[1];
	NSInteger nColBuf = (colBuf[0] == nil)? 0 : colBuf[0].length / sizeof(vector_float4);
	...
	if (nColBuf != newNC) {
		if (newNC > 0) {
			...
			newColBuf[1] = [view.device newBufferWithLength:
				sizeof(vector_float4) * newNC options:MTLResourceStorageModeShared];
			if (newColBuf[0] == nil || newColBuf[1] == nil) {
				error_msg(@"Could not allocate buffer for colors.", nil);
				@throw @(FailColBuf);
			}
		} else colBuf[0] = newColBuf[0] = colBuf[1] = newColBuf[1] = nil;
	}
}
- (void)adjustVxBufferSize:(id<MTLBuffer> *)newVxBuf {
	newVxBuf[0] = vxBuf[0]; newVxBuf[1] = vxBuf[1];
	NSInteger nVertices = (vxBuf[0] == nil)? 0 : vxBuf[0].length / sizeof(vector_float2);
	...
	if (newNV != nVertices) {
		...
		newVxBuf[1] = [view.device newBufferWithLength:
			sizeof(vector_float2) * newNV options:MTLResourceStorageModeShared];
		if (newVxBuf[0] == nil || newVxBuf[1] == nil) {
...
- (BOOL)adjustMemoryForNParticles {
	[bufLockD lock]; [bufLockR lock];
	BOOL result = YES;
	@try {
		id<MTLBuffer> __autoreleasing newColBuf[2], newVxBuf[2];
		Particle *newPTCLs = [self adjustPtclMemory];
		[self adjustColBufferSize:newColBuf];
		[self adjustVxBufferSize:newVxBuf];
		_particles = newPTCLs; _nPtcls = NParticles;
		colBuf[0] = newColBuf[0]; colBuf[1] = newColBuf[1];
		vxBuf[0] = newVxBuf[0]; vxBuf[1] = newVxBuf[1];
		[self setupVertices];
	...
	[bufLockR unlock]; [bufLockD unlock];
	return result;
}
- (void)requestRedering {
	if (renderCompleted) {
		if (cBufModified)
			{ cBufIdxR = cBufIdxD; cBufIdxD = 1 - cBufIdxD; cBufModified = NO; }
		if (vBufModified)
			{ vBufIdxR = vBufIdxD; vBufIdxD = 1 - vBufIdxD; vBufModified = NO; }
		renderCompleted = NO;
	}
...
- (instancetype)initWithView:(MTKView *)mtkView agent:(Agent *)a {
	...
	[NSNotificationCenter.defaultCenter addObserverForName:keyShouldRedraw
		...
		[self requestRedering];
	}];
	[NSNotificationCenter.defaultCenter addObserverForName:@"colorParticles"
		object:NSApp queue:nil usingBlock:^(NSNotification * _Nonnull note) {
		[self->bufLockD lock];
		[self setupParticleColors];
		[self->bufLockD unlock];
		if (self.displayMode == DispParticle) [self requestRedering];
	}];
	[NSNotificationCenter.defaultCenter addObserverForName:@"nParticles"
		...
		else if (self.displayMode == DispParticle) [self requestRedering];
	}];
	...
	[NSNotificationCenter.defaultCenter addObserverForName:keyColorMode
		...
		[self->bufLockD lock]; [self->bufLockR lock];
		@try {
			id<MTLBuffer> __autoreleasing colBuf[2];
			[self adjustColBufferSize:colBuf];
			self->colBuf[0] = colBuf[0]; self->colBuf[1] = colBuf[1];
			[self setupParticleColors];
		} @catch (id x) {
			[(ControlPanel *)note.userInfo[keyCntlPnl] adjustColorMode:note.userInfo];
		}
		[self->bufLockR unlock]; [self->bufLockD unlock];
		[self requestRedering];
	}];
	[NSNotificationCenter.defaultCenter addObserverForName:keyDrawMethod
		...
		[self->bufLockD lock]; [self->bufLockR lock];
		@try {
			id<MTLBuffer> __autoreleasing vxBuf[2];
			[self adjustVxBufferSize:vxBuf];
			self->vxBuf[0] = vxBuf[0]; self->vxBuf[1] = vxBuf[1];
			[self setupVertices];
		} @catch (id x) {
...
- (void)setupParticleColors {
	if (colBuf[0] == nil) return;
	vector_float4 *colors = colBuf[cBufIdxD].contents, ptclHSB = ptcl_hsb_color();
...
- (void)setupVertices {
	int nVpL = (int)(vxBuf[0].length / sizeof(vector_float2) / _nPtcls);
	vector_float2 *lines = vxBuf[vBufIdxD].contents;
...
- (void)setupArrows:(RCE)rce n:(int)n {
	...
	memcpy(vxBuf[vBufIdxR].contents, _arrowVec, sizeof(vector_float2) * n * nv);
	memcpy(colBuf[cBufIdxR].contents, _arrowCol, sizeof(vector_float4) * n);
	[rce setVertexBytes:&nv length:sizeof(nv) atIndex:IndexNVforP];
	[rce setVertexBuffer:vxBuf[vBufIdxR] offset:0 atIndex:IndexVertices];
	[rce setVertexBuffer:colBuf[cBufIdxR] offset:0 atIndex:IndexColors];
	...
	switch (_displayMode) {
		case DispParticle: if (_particles != NULL) {
			uint nVertices = (vxBuf == NULL)? 0 :
				(uint)(vxBuf[0].length / sizeof(vector_float2));
			if (colBuf[0] != nil) {
...
- (void)oneStepForParticles {
	...
	for (int i = 0; i < _nPtcls; i += unit) {
		...
		[opeQue addOperationWithBlock:^{
			for (int j = 0; j < unit; j ++) {
				Particle *p = pStart + j;
				int ix = floor(p->p.x / TileSize), iy = floor(p->p.y / TileSize);
...
- (void)oneStep {
	...
	in_main_thread(^{ [self requestRedering]; });
	[bufLockD unlock];
}
