...
- (NSMutableData *)adjustColBufferSize {
	NSMutableData *newColMem = colMem;
	NSInteger orgNC = colMem.length / sizeof(vector_float4),
		newNC = (_displayMode == DispParticle)?
			(ptclColorMode == PTCLconstColor)? 0 : NParticles :
			(_displayMode == DispVector)? N_VECTORS : NActiveGrids * NActs;
	if (orgNC != newNC) {
		if (newNC > 0) {
			newColMem = [NSMutableData dataWithLength:sizeof(vector_float4) * newNC];
			if (newColMem == nil) {
...
- (NSMutableData *)adjustVxBufferSize {
	NSMutableData *newVxMem = vxMem;
	NSInteger orgNC = vxMem.length / sizeof(vector_float2),
		newNV = (_displayMode == DispParticle)?
	...
	if (orgNC != newNV) {
		newVxMem = [NSMutableData dataWithLength:sizeof(vector_float2) * newNV];
		if (newVxMem == nil) {
...
- (BOOL)adjustMemoryForNParticles {
	...
	@try {
		...
		NSMutableData *newColMem = [self adjustColBufferSize];
		NSMutableData *newVxMem = [self adjustVxBufferSize];
...
- (instancetype)initWithView:(MTKView *)mtkView agent:(Agent *)a {
	...
	[NSNotificationCenter.defaultCenter addObserverForName:keyColorMode
		...
		[self->vxBufLock lock];
		@try {
			self->colMem = [self adjustColBufferSize];
			[self setupParticleColors];
		} @catch (id x) {
	...
	[NSNotificationCenter.defaultCenter addObserverForName:keyDrawMethod
		...
		[self->vxBufLock lock];
		@try {
			self->vxMem = [self adjustVxBufferSize];
			[self setupVertices];
		} @catch (id x) {
...
- (void)setupParticleColors {
	if (colMem == nil) return;
	vector_float4 *colors = colMem.mutableBytes, ptclHSB = ptcl_hsb_color();
	...
	colBuf = [view.device newBufferWithBytes:colMem.bytes
		length:colMem.length options:MTLResourceStorageModeManaged];
}
- (void)setupVertices {
	int nVpL = (int)(vxMem.length / sizeof(vector_float2) / _nPtcls);
	vector_float2 *lines = vxMem.mutableBytes;
	...
	[opeQue waitUntilAllOperationsAreFinished];
	vxBuf = [view.device newBufferWithBytes:vxMem.bytes
		length:vxMem.length options:MTLResourceStorageModeManaged];
...
- (void)setupBuffersForArrows:(int)n {
	vxBuf = [view.device newBufferWithBytes:_arrowVec
		length:sizeof(vector_float2) * n * NVERTICES_ARROW
		options:MTLResourceStorageModeManaged];
	colBuf = [view.device newBufferWithBytes:_arrowCol
		length:sizeof(vector_float4) * n options:MTLResourceStorageModeManaged];
}
- (void)setupArrowsForVecFld {
	...
	[self setupBuffersForArrows:N_VECTORS];
}
- (void)setupArrowsForQValues {
	...
	[self setupBuffersForArrows:NActiveGrids * NActs];
}
...
- (void)drawScene:(nonnull MTKView *)view commandBuffer:(id<MTLCommandBuffer>)cmdBuf {
	...
	switch (_displayMode) {
		case DispParticle: if (_particles != NULL) {
			uint nVertices = (vxMem == nil)? 0 :
				(uint)(vxMem.length / sizeof(vector_float2));
			if (colMem != nil) {
